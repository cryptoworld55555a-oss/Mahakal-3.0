...
const PROTOCOL_ABI = [
  ...
  "function claimMerkle(uint256 cumulativeUsd, bool capReduce, uint256 minTtnOut, uint256 deadline, bytes32[] proof)",
...
// Fetch this user's reward breakdown + Merkle proofs, then claim each stream on-chain.
export async function claimAllRewards(address) {
  ...
  for (const leaf of proofs) {
    const tx = await p.claimMerkle(leaf.amount_wei, leaf.capReduce, 0, deadline(), leaf.proof);
    await tx.wait();
    hashes.push(tx.hash);
  }
  return hashes;
