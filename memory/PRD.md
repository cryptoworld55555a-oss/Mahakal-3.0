...
## ✅ POOL SPLIT FIX + POOL_LIVE FIX + LOGO (2026-06)
- LIQUIDITY SEEDED: user ran seedLiquidity tx 0xf5129b... SUCCESS. Pair 0x9eab306de1baac989078a5ad53b1334dc3e7ef60. 20,000 TTN + 200 USDT. Price $0.01/TTN exact. LP 100% burned to dead addr. Pool permanent.
- BUG: onchain.py read TOKEN_ADDRESS at module import (line 12) BEFORE server.py load_dotenv() (line 26) -> TOKEN=zero -> pool_live always False. FIX: made onchain.py read TOKEN/RPC at call-time (_token_address(), _rpc_url()). Verified preview pool_live=True price=0.01.
- BUG: /activate split was WRONG (20% creator + 15/15/15 pools + 15% community = only 80%). CORRECT per user Rule Book 5.0: 60% TTN stake + 5% dev + 5% daily + 5% weekly + 25% level income = 100%. Monthly pool NOT from activation (comes from 10% deduction on daily+weekly PAYOUTS). Community fund = 0 from activation. reward_engine.py was ALREADY correct (DAILY/WEEKLY_POOL_BPS=500, LEVEL_BPS sum=2500, MONTHLY_DEDUCT_BPS=1000). Fixed server.py /activate to match. Added level_income_pool_usdt + ttn_stake_usdt to DEFAULT_STATS. Made dashboard_stats use .get() (was KeyError/500 on missing monthly/community). Verified $10 activation -> stake 6.0, dev 0.5, daily 0.5, weekly 0.5, level 2.5, monthly 0, community 0 = 100%.
- FRONTEND BUG: GlobalBusiness.jsx line 17 used usd() (0 decimals) for pool amount -> $1.5 shown as $2. Fixed to usd2() (2 decimals).
- MINING CAP decision (user chose): cap = 200% of FULL activation amount ($200 on $100), NOT 200% of 60% stake. Keep as-is (total_deposited = full amount). No change made.
...
## ✅ BINARY LEFT/RIGHT REFERRAL LINKS (2026-06)
- ISSUE: referral link only had /?ref=<code> (single). Binary tree needs left/right leg placement. Backend had binary_parent/binary_side fields + tree_engine leg logic, BUT registration never set them (only sponsor). So left/right legs always empty -> monthly qualification (25/25 IDs, $5000/leg) could never fill.
- FIX backend: VerifyRequest now has `leg` field. Added _place_binary(sponsor, leg) helper = spillover placement (walks DOWN chosen leg until empty slot). /auth/verify now sets binary_parent + binary_side. sponsor (direct/level income) stays separate from binary placement (leg business) - correct binary MLM.
- FIX frontend: WalletContext captures ?leg= param (stores titan_leg, sends in verifySignature). MyTeamPage now shows TWO links: Left Leg (&leg=left) + Right Leg (&leg=right) with separate copy buttons (testids team-referral-copy-left / -right).
...
## ✅ PER-USER CLAIM COUNTDOWN (Part A partial) — 2026-06
- USER REQUIREMENT: claim button must stay LOCKED until a per-user reverse countdown ends. Countdown starts from the user's OWN activation time (activated_at): Daily = 24h, Weekly = 7d (168h), Monthly = 30d (720h). Ulta countdown 24:00:00 -> 00:00:00, then claim opens.
- BACKEND (server.py): added _claim_window(activated_at, period_hours) helper; /pools/{address} now returns claim:{open, unlock_at, seconds_left, activated_at} for daily(24h)/weekly(168h)/monthly(720h). If not activated -> open:false. VERIFIED: user activated 1h ago -> daily locked ~23h; user activated 25h ago -> daily open:true.
- FRONTEND (PoolsPage.jsx): live 1s ticker; PoolCard shows countdown box ("Claim opens in HH:MM:SS" + "Opens at [date]") while locked, "Claim window is OPEN" when done. Button disabled + labelled "Locked · HH:MM:SS" until open; doClaim() also guards (toast "Claim is locked"). Before activation -> "Activate to start".
- STILL PENDING (full Part A): server-side / on-chain enforcement (merkle leaf still includes pool share regardless of countdown — the countdown is currently a UI + /pools gate, NOT enforced in tree_engine/merkle). Also: freeze cycle qualifiers + equal split + pool reset + cron scheduler. Countdown fix directly addresses the reported "claimed before countdown" UI bug but on-chain merkle gating must still be wired before real pool claims.
- PENDING DEPLOY: server.py + PoolsPage.jsx -> git pull + restart titan-backend + frontend yarn build.

## ✅ POOL CYCLE SETTLEMENT + CLAIM LOCK (Part A DONE) — 2026-06
- MODEL (user-confirmed, matches dashboard reset countdown): daily/weekly/monthly pool claims are LOCKED while the shared cycle countdown runs. At cycle CLOSE: freeze current qualifiers, split the pool EQUALLY, credit each qualifier's cumulative settled_*_usd, RESET pool to 0. THEN claim opens showing each user's frozen share.
- CYCLE IDS: daily=%Y-%m-%d, weekly=%G-W%V, monthly=%Y-%m. Cycle boundary = _pool_resets() (daily=next midnight UTC, weekly=next Mon, monthly=1st). When cycle id advances, previous cycle settles.
- BACKEND (server.py):
  - _cycle_ids(now), _settle_due_pools(): idempotent per cycle via db.pool_state{_id:cycles, daily_open/weekly_open/monthly_open}. daily/weekly: recipients get 90%, 10% -> monthly_pool_usdt (accumulates); no qualifiers -> whole balance carries to monthly. monthly: full split. Records db.pool_settlements. Resets pool to 0.
  - _load_network carries settled_daily/weekly/monthly_usd. _rebuild_snapshot STRIPS live pool leaves (cat 2/3/4) and re-adds leaves from SETTLED cumulative only -> pool rewards NOT in merkle (not claimable on-chain) until a cycle closes. Adds settled_*_usd + settled_claimable_usd to breakdown.
  - _pool_claim(pool, settled_usd, resets) -> {settled_usd, open(settled>0), unlock_at(next close), seconds_left}.
  - /pools + /dashboard/stats call _settle_due_pools() (lazy) then rebuild if changed. New endpoint POST /reward/settle-due (for cron, idempotent).
- FRONTEND (PoolsPage.jsx): countdown to unlock_at while locked ("Locked · HH:MM:SS", button disabled). After settlement shows "Your qualified share $X" + button "Claim $X". doClaim guard rejects if !open.
